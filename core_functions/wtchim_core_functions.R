...
load_data = function(normalise = TRUE, remove_doublets = FALSE, remove_stripped = FALSE, load_corrected = FALSE){
  ...
  counts = readRDS("/nfs/research1/marioni/jonny/chimera-wt/data/raw_counts.rds")
  genes = read.table("/nfs/research1/marioni/jonny/chimera-wt/data/genes.tsv", stringsAsFactors = F)
  meta = read.table("/nfs/research1/marioni/jonny/chimera-wt/data/meta.tab", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "$")
  ...
  if(normalise){
    sfs = read.table("/nfs/research1/marioni/jonny/chimera-wt/data/sizefactors.tab", stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs
  ...
  if(load_corrected){
    corrected = readRDS("/nfs/research1/marioni/jonny/chimera-wt/data/corrected_pcas.rds")
